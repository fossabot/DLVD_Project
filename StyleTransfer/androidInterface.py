...
def export_gen_graph(tf, sess, variables_filter, variables_bias, variables_scalars, path, name="gen_export.pb", resolution=224) :
    ...
    with to_graph.as_default() as g:
        gn.build_gen_graph_deep(tf, trainable=False, variables_gen_filter=var_gen_filter_new, variables_gen_bias=var_gen_bias_new, variables_scalars=var_gen_scalars_new, input_resolution=resolution)
...
def test_android_gen(tf):
    ...
    with open(conf.project_path + full_path + '\\gen_export_712.pb', mode='rb') as f:
        fileContent = f.read()

    graph_def = tf.GraphDef()
    graph_def.ParseFromString(fileContent)

    input_image = tf.placeholder("float32", (1, 712, 712, 3), "input")
    ...
    feed = {}
    feed[input_image] = content.reshape(1, 712, 712, 3)
