...
def export_gen_graph(tf, sess, variables_filter, variables_bias, variables_scalars, path, name="gen_export.pb", resolution=224, ratio=1.0) :
    ...
    with to_graph.as_default() as g:
        gn.build_gen_graph_deep(tf, trainable=False, variables_gen_filter=var_gen_filter_new, variables_gen_bias=var_gen_bias_new, variables_scalars=var_gen_scalars_new, input_resolution=resolution, ratio=ratio)
...
def export_checkpoint_to_android(tf):
    ...
    with tf.Session() as sess:
        ...
        export_gen_graph(tf, sess, variables_gen_filter, variables_gen_bias, variables_scalars, saving_directory,
                         name='gen_export_832_468.pb', resolution=468, ratio=16.0/9.0)
...
def test_android_gen(tf):
    ...
    content, avg_content_gen = utils.load_image("\\chicago.jpg", between_01=True, substract_mean=False, output_size=468, ratio=16.0/9.0)

    print('load generator')
    with open(conf.project_path + full_path + '\\gen_export_832_468.pb', mode='rb') as f:
        fileContent = f.read()

    graph_def = tf.GraphDef()
    graph_def.ParseFromString(fileContent)

    input_image = tf.placeholder("float32", (1, 832, 468, 3), "input")
    ...
    feed = {}
    feed[input_image] = content.reshape(1, 832, 468, 3)
